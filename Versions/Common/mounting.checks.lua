...
function BeStride:Druid()
	...
	elseif IsSwimming() and self:DruidCanSwim() and BeStride:DBGet("settings.mount.noswimming") == false then
		return BeStride_Mount:MountSpell(SpellToName(BeStride_Constants.spells.druid.aquaticform))
	elseif self:MovementCheck() and IsOutdoors() then
		return BeStride_Mount:MountSpell(SpellToName(BeStride_Constants.spells.druid.travelform))
	...
	elseif IsMounted() and IsFlying() and self:IsFlyable() and self:DruidCanFly() and self:DruidFlyingMTFF() then
		return BeStride_Mount:MountSpell(SpellToName(BeStride_Constants.spells.druid.flightform))
	...
	elseif IsFalling() and self:DruidCanFly() then
		return BeStride_Mount:MountSpell(SpellToName(BeStride_Constants.spells.druid.flightform))
	elseif IsMounted() ~= true and IsFlying() and BeStride:DBGet("settings.mount.nodismountwhileflying") ~= true then
		return BeStride_Mount:MountSpell(SpellToName(BeStride_Constants.spells.druid.flightform))
	else
